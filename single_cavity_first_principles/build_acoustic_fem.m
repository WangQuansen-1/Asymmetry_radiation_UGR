...
if cfg.femOrder==1
    conn=tet;n=nVertex;[Kbase,M]=assemble_p1(conn,grad,vol,n);
    allEdges=[];
else
    ...
    [Kbase,M]=assemble_p2(conn,grad,vol,n);
end
...
sys=struct('kind','fem','K',K,'Kbase',Kbase,'M',M,'B',B, ...
    'vertex',v,'tet',uint32(conn),'n',n,'cfg',cfg);
fprintf('FEM-P%d: %d DOFs, %d tetrahedra, %d radiation triangles\n', ...
    cfg.femOrder,n,ne,size(tri,2));
end
...
function [K,M]=assemble_p2(conn,g,vol,n)
...
for first=1:chunk:ne
    ...
    for q=1:4
        ...
        for i=1:10
            for j=1:10
                r=r+1;Ke(r,:)=Ke(r,:)+0.25*vol(e).*sum(G(:,:,i).*G(:,:,j),1);
            end
        end
    end
    Me=Mref(:).*vol(e);
